...
def vid_down(link):
    ...
    vid = YouTube(link).streams.filter(file_extension='mp4',progressive=True).first().download('music')
...
def play_down(link):
    ...
    for url in p.video_urls:
        try:
            ...
            yt.streams.filter(progressive=True,file_extension='mp4').first().download("music")
            i=i+1
